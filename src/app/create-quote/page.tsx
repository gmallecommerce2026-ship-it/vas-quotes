...
// Hàm tạo mã ID tự động cho khách hàng
const generateSlug = (str: string) => {
  if (!str) return "pmh";
  return str.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().trim().replace(/[^a-z0-9\s]/g, "").replace(/\s+/g, "_");
};
...
export default function CreatePurchaseOrderPage() {
  ...
  const [paymentMethod, setPaymentMethod] = useState("Chuyển khoản / Tiền mặt");
  ...
  const handleNativeShare = async () => {
    ...
    messageText += `\nMã phiếu: ${autoCustomerId}`;

    if (navigator.share) {
      try { await navigator.share({ title: `Phieu_Mua_Hang_${customerName || 'KH'}`, text: messageText }); } catch (e) { console.log(e); }
        ...
        <div className="max-w-[800px] mx-auto bg-white p-4 sm:p-8 shadow-sm print:shadow-none print:w-full">
          ...
          <div className="text-black font-sans text-[13px] print:text-[12px] leading-relaxed">
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-2 mb-6 text-sm">
              ...
              <div className="flex gap-2 col-span-2"><span className="font-semibold w-[140px] shrink-0">Hình thức thanh toán:</span><span className="border-b border-dotted border-gray-400 flex-1">{paymentMethod}</span></div>
            </div>

            {/* BẢNG IN XUẤT ĐÃ ĐƯỢC CHỈNH LẠI GIAO DIỆN ĐẸP MẮT */}
            <div className="w-full overflow-x-auto custom-scrollbar border border-slate-300 rounded-xl">
  ...
  return (
    <div className="px-4 space-y-5 pb-32">
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border space-y-3">
        ...
        <input placeholder="Hình thức thanh toán" value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)} className="w-full text-sm p-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-blue-500" />
      </div>
